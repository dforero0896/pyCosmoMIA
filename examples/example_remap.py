...
if __name__ == '__main__':
    ...
    fig.savefig("plots/example_remap.png", dpi=300)
    #cmap = "Dense"
    ...
    for kk, bs in enumerate([200, 800, 1200]):
        interp_smooth = 5
        box_size = [bs] * 3
        ...
        bins = jnp.geomspace(1e-1, 100, 100)
        counts, bins, p = ax[0].hist(2 + delta_ev_pm.ravel(), bins = bins, histtype = 'step', density = True, color = f"C{kk}")
        fig.savefig("plots/example_remap.png", dpi=300)   
    
    
        tic = time.time()
        psi = aug_lpt(delta_lin, box_size[0], smooth, interp_smooth, sc_alpha = 3. / 2)
        ...
        n = int(np.ceil(20 * 0.78))
        print(n)
        m = ax[kk+1].imshow(jnp.log10(2 + delta_ev[:20, :, :].mean(axis=0)), colorbar = None, vmin = VMIN, cmap = cmap, vmax = VMAX)
        ax[kk+1].format(title = rf'$\Delta x = {resolution[0]:.1f}$', xlocator = 'null', ylocator = 'null', xlabel = f"$L = {bs}~\mathrm{{Mpc}}/h$")
        #ax[0].semilogx(k_, pk_)#, label = 'SC')
        counts, bins, _ = ax[0].hist(2 + delta_ev.ravel(), bins = bins, histtype = 'step', density = True, color = f"C{kk}", ls ='--')
        ax[4].semilogx(k_, r_, label = rf'$\Delta x = {resolution[0]:.1f}$')
        del delta_ev
        fig.savefig("plots/example_remap.png", dpi=300)
    ...
    fig.colorbar(m, label = "$\log(\delta + 2)$", loc = 'bottom', cols = (2,4))
