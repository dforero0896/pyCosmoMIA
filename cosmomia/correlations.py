...
def naive_xpk(delta1, delta2, box_size, k_edges):
    ...
    kx = jnp.fft.fftfreq(delta1.shape[0], box_size / dims) * 2 * jnp.pi
    ky = kx.copy()
...
def naive_rcoeff(delta1, delta2, box_size, k_edges):
    ...
    pk_1 = counts_auto_1 / modes_auto_1 * (box_size / dims**2)**3
    pk_2 = counts_auto_2 / modes_auto_2 * (box_size / dims**2)**3
    
    
    return bins[:-1] + 0.5 * jnp.diff(bins), xpk / jnp.sqrt(pk_1 * pk_2)
